...
file1 <- "QT/morphine_codeine/long_term_more_10_exposures/burden_test/phenos.QT.step2_total_exposures_not_injectable.regenie"
file2 <- "QT/morphine_codeine/long_term_more_10_exposures/burden_test/phenos.QT.step2_PDC_adherence_score.regenie"
file3 <- "QT/morphine_codeine/long_term_more_10_exposures/burden_test/phenos.QT.step2_MPR_adherence_score.regenie"
```
...
```{r}
phenotype2 <- process_file(file2, "PDC adherence score")
```
...
overlap_info <- read_tsv("QT/morphine_codeine/long_term_more_10_exposures/burden_test/mask_phenotype_long_term_users_QT_overlap.tsv", show_col_types = FALSE)
...
forest_plot <- ggplot(combined_df, aes(x = BETA, y = ID_label)) +
  ...
  labs(
    title = "Forest plot quantitave phenotypes \n(Codeine/Morphine users with >= 10 exposures)",
    subtitle = "N = 21678",
    x = "Effect Size",
    y = "Masks"
  ) 
...
ggsave("QT/morphine_codeine/long_term_more_10_exposures/burden_test/forest_plot_QT_phenotypes_morphine_codeine_long_term_users_10_exposures.png", forest_plot, width = 10, height = 10 , dpi = 300, bg = "white")
```
...
PDC_df <- combined_df %>%
  filter(Phenotype == "PDC adherence score")

# Create the forest plot with consistent style
forest_plot <- ggplot(PDC_df, aes(x = BETA, y = ID_label)) +
  geom_errorbarh(aes(xmin = lower, xmax = upper), height = 0.2, color = "grey40") +
  ...
  labs(
    title = "Forest plot quantitave phenotype: PDC adherence score 
    \n(Codeine/Morphine users with >= 10 exposures)",
    subtitle = "N = 21678",
    x = "Effect Size",
    y = "Masks"
...
```{r}
# Save the plot with bigger dimensions
ggsave("QT/morphine_codeine/long_term_more_10_exposures/burden_test/forest_plot_PDC_score_QT_phenotype_morphine_codeine_long_term_users_10_exposures.png", forest_plot, width = 10, height = 6 , dpi = 300, bg = "white")
```
